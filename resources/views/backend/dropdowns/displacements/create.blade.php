					...
					<div class="box-body">
						<div class="row pt-2">
							<div class="col-12"><!--left col-->
								<form action="@if(isset($displacement)) {{ route('displacements.update', $displacement->id) }} @else {{ route('displacements.store') }} @endif" method="post" enctype="multipart/form-data">
									@csrf
									...
									<div class="form-group">
										...
										<select id="model" name="model_id" class="custom-select" derive-from="category">
											...
											<option value="{{ $model->id }}" @if(isset($displacement) && $model->id == $displacement->model_id) selected @endif derive-parent="{{ $model->category_id }}">{{ $model->name }}</option>
											@endforeach
										</select>
									</div>
									<div class="form-group">
